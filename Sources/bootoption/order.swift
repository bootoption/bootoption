...
let order = Command("order", helpMessage: "change the boot order") {
        ...
        let parser = OptionParser(Options.printBootOrder, helpName: "order", invocation: invocationMessage, parserOptions: [.allowUnparsedOptions])
        ...
        guard !bootOrder.isEmpty else {
                let errorMessage = "the boot order is empty or not set and cannot be reordered"
